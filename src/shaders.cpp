...
RID nv12(RenderingDevice *rd, Vector2i size) {
	...
	String s = R"(
		...
		void main() {
			...
			float y = imageLoad(Y, texel).r;
			float u = imageLoad(UV, texel_u).r;// - .5;
			float v = imageLoad(UV, texel_v).r;// - .5;
	...
	auto spirv = rd->shader_compile_spirv_from_source(src);
	cache = rd->shader_create_from_spirv(spirv, "avg_nv12");
	if (!cache.is_valid()) {
		UtilityFunctions::printerr("YUV420 shader compile error: ",
				spirv->get_stage_compile_error(RenderingDevice::SHADER_STAGE_COMPUTE));
		return {};
	}
	UtilityFunctions::print("created nv12 shader");
	return cache;
}