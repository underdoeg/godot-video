...
auto k_use_vk_decoders = "use_vk_decoders";
...
void init() {
	...
	{
		const auto key = p(k_use_vk_decoders);
		if (!ps->has_setting(key)) {
			ps->set_setting(key, false);
		}
		ps->set_initial_value(key, false);
...
bool use_threads() {
	return ProjectSettings::get_singleton()->get_setting(p(k_threads), false) && !use_vk_decoders();
}
...
bool use_vk_decoders() {
	return ProjectSettings::get_singleton()->get_setting(p(k_use_vk_decoders), false);
}
